...
class DataGrid
{
    ...
    public function DataSourceNew($params = array()){
        ...
        if($this->dbType == 'PEAR'){            
            ...
            /* Ex.: 'datagrid/pear/' */    
            if(!defined('PEAR_DIR'))
                define('PEAR_DIR', $this->directory . 'modules/pear/');
    ...
    protected function DrawTabular(){
        ...
        $this->RowOpen('');
            ...
            if($this->ModeAllowed('edit')){
                $this->MainColOpen('center',0,'nowrap', "2%", $this->cssClass."_dg_th_normal"); echo $this->lang['edit']; $this->MainColClose();                
            }
        ...
        for($r = $this->rowLower; (($r >=0 && $this->rowUpper >=0) && ($r < $this->rowUpper) && ($r < ($this->rowLower + $this->reqPageSize))); $r++){            
                ...
                if(($this->mode == 'edit') && $this->GetFieldOffset($this->primaryKey) != '-1' && (intval($this->rid) == intval($row[$this->GetFieldOffset($this->primaryKey)]))){
                    ...
                    if($this->ModeAllowed('edit')){
                        ...
                        if(isset($this->modes['edit']['byFieldValue']) && ($this->modes['edit']['byFieldValue'] != '')){
                            ...
                            if($this->ModeAllowed('add')) $this->ColOpen('center',0,'nowrap',$main_td_color, $this->cssClass."_dg_td_main", "8%");
            ...
            if($this->rowsNumeration){
                $page_num = (($this->GetVariable('p') != '') ? $this->GetVariable('p') : '1');        
                $rows_numeration = (($page_num - 1)* $this->reqPageSize) +  ($r + 1);
                $this->ColOpen('center',0,'nowrap',$main_td_color, $this->cssClass."_dg_td_main", "8%");/*$this->ColOpen('center',0,'nowrap');*/ echo "<label class='".$this->cssClass."_dg_label'>".$rows_numeration."</label>"; $this->ColClose();
            }
