...
<script>
    $(document).ready(function () {
        $("#btnSave").click(function () {
            var url = "<?php echo $webUrl; ?>crm/companyHistoryEdit/<?php echo $id; ?>/<?php echo $company_id?>";
            if (validateFrom(document.getElementById('frmHistoryPost'))) {
                $.post(url, $("#frmHistoryPost").serialize(),
                    function (result) {
                        if (result == "add fail") {
...
<div class="block">
    ...
    <div class="block-content collapse in">
        <form id="frmHistoryPost" name="frmHistoryPost" method="post" action="">
            ...
            <div class="row-fluid">
                ...
                <div class="span4">
                    <input class="input-block-level"
                           value="<?php echo $create_time; ?>" disabled/>
                </div>
            </div>
            <div class="row-fluid">
                <div class="span2">หัวข้อ</div>
                <div class="span4">
                    <input name="title" type="text" id="title" class="input-block-level" value="<?php echo $title;?>"/>
                </div>
            </div>
            <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <div class="span4">
                    <input type="radio" name="priority" value="3" <?php echo $priority==3?'checked':''; ?>>มาก
                    <input type="radio" name="priority" value="2" <?php echo $priority==2?'checked':''; ?>> ปานกลาง
                    <input type="radio" name="priority" value="1" <?php echo $priority==1?'checked':''; ?>> น้อย
                </div>
            </div>
            <div class="row-fluid">
                ...
                <div class="span4">
                    <select name="key_account_manager_id" id="key_account_manager_id" class="input-block-level">
                        ...
                        <?php foreach ($arrEmployee as $key => $value): ?>
                            <option <?php echo $value->id == $key_account_manager_id ? 'selected': '';?>
